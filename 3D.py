...
TRAIL = 150
STEP  = 3
N_SEG = 20        # number of fade segments per trail
DT    = 0.01      # time between each saved row in your CSV — adjust to match your simulation
...
T_total = N * DT   # total simulated time

# timestep + period info box — upper right
...
# fading trail segments — alpha ramps from faint to full toward current position
trail_segs0 = [ax.plot([], [], [], color="blue",     lw=1.5, alpha=(i+1)/N_SEG)[0] for i in range(N_SEG)]
trail_segs1 = [ax.plot([], [], [], color="deeppink", lw=1.5, alpha=(i+1)/N_SEG)[0] for i in range(N_SEG)]
trail_segs2 = [ax.plot([], [], [], color="green",    lw=1.5, alpha=(i+1)/N_SEG)[0] for i in range(N_SEG)]
...
# legend
legend_elements = [
    Line2D([0], [0], color='blue',     lw=1.5, label='body 0'),
    Line2D([0], [0], color='deeppink', lw=1.5, label='body 1'),
    Line2D([0], [0], color='green',    lw=1.5, label='body 2'),
    Line2D([0], [0], marker='^', color='w', markerfacecolor='darkblue',  ms=8, label='start 0'),
    Line2D([0], [0], marker='^', color='w', markerfacecolor='red',        ms=8, label='start 1'),
    Line2D([0], [0], marker='^', color='w', markerfacecolor='darkgreen',  ms=8, label='start 2'),
]
