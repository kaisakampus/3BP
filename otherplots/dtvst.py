...
# load timestep sizes
timesteps_main = np.loadtxt(r"C:\Users\kaisa\My Drive\Simulated_Data\brouckeA2_NEW_timestep_sizes.csv")
timesteps_sub  = np.loadtxt(r"C:\Users\kaisa\My Drive\Simulated_Data\brouckeA2add_NEW_timestep_sizes.csv")
timesteps_add  = np.loadtxt(r"C:\Users\kaisa\My Drive\Simulated_Data\brouckeA2sub_NEW_timestep_sizes.csv")
...
plt.plot(cumulative_time_main[1:], timesteps_main[1:], color="deeppink",         linewidth=1, label="broucke A2 r1_x")
plt.plot(cumulative_time_sub[1:],  timesteps_sub[1:],  color="turquoise", linewidth=1, label="broucke A2 (r1_x-ε)")
plt.plot(cumulative_time_add[1:],  timesteps_add[1:],  color="lime",    linewidth=1, label="broucke A2 (r1_x+ε)")
plt.xlim(left=0)
#plt.axvline(x=55, color="black", linewidth=1, linestyle="--")
plt.xlabel("simulation time")
plt.ylabel("adaptive timestep size dt")
plt.title("broucke A2 ±ε=1e-4")
plt.legend()
